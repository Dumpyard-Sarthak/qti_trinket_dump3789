...
function initOplusLog(){
    ...
    echo "${CURTIME_FORMAT} init oplus log: start..." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    ...
    if [ "${PANICE_NABLE}" = "true" ] || [ x"${CAMERA_PANIC_ENABLE}" = x"true" ]; then
        ...
        traceTransferState "init:start debug time: ${CURTIME}"
...
function transfer2SDCard(){
    ...
    stoptime=`getprop sys.oppo.log.stoptime`;
    traceTransferState "${LOGTAG}:start...."
    newpath="${SDCARD_LOG_BASE_PATH}/log@stop@${stoptime}"
    mkdir -p ${newpath}
    traceTransferState "${LOGTAG}:from ${DATA_LOG_PATH} to ${newpath}"
...
function transfer_log(){
    traceTransferState "transfer:start...."
    setprop ctl.start dump_system

    transfer2SDCard
    #transferLog
    traceTransferState "transfer:done...."
